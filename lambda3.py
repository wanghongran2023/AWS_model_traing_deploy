import json
import ast

THRESHOLD = .93


def lambda_handler(event, context):
    
    # For Testing in AWS Console (parse event depending on structure)
    if isinstance(event, str):
        event_tmp = json.loads(event['body'])
    else:
        event_tmp = event['body']
    event_json=json.loads(event_tmp)
        
    # Grab the inferences from the event
    inferences = event_json["inferences"]
    inferences = ast.literal_eval(inferences)
    
    # Check if any values in our inferences are above THRESHOLD
    meets_threshold = float(inferences[0]) > THRESHOLD or float(inferences[1]) > THRESHOLD
    
    # If our threshold is met, pass our data back out of the
    # Step Function, else, end the Step Function with an error
    if meets_threshold:
        pass
    else:
        raise("THRESHOLD_CONFIDENCE_NOT_MET")

    return {
        'statusCode': 200,
        'body': json.dumps(event)
    }

# Test Data{
#  "statusCode": 200,
#  "body": "{\"statusCode\": 200, \"body\": {\"image_data\": \"\", \"s3_bucket\": \"modeltrainwang\", \"s3_key\": \"test/bicycle_s_000030.png\", \"inferences\": []}, \"inferences\": \"[0.005733632482588291, 0.9942663311958313]\"}"
#}